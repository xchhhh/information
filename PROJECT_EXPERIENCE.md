...
### 质量数据（确定性实测，10 条真实问答对，语料 6 个段落）

| 指标 | 含义 | 优化前(top_k=4) | 优化后(top_k=8) | 提升 |
|------|------|----------------|----------------|------|
| 相关段落覆盖率 | 最终上下文覆盖"向量最相似 top-10 段落"的比例 | 66.7% | 100.0% | +50.0% |
| 平均检索耗时 | 单次混合检索延迟(ms) | 310.6 | 310.6 | — |

> 实测环境：腾讯云服务器（2026-07-19，运行 `python -m evaluation.benchmark`）。
> 覆盖率@4 仅 66.7% 的原因：在 6 个段落的小语料上，top_k=4 会漏掉约 1/3 的相关块；扩到 top_k=8 后窗口足以覆盖全部相关段落，覆盖率达 100%，且因服务器首次检索已缓存、混合检索耗时未增加（仍为 310.6ms）。
